...
const RESOURCE_ROOT = fileURLToPath(new URL("../mod-fabric/src/main/resources/", import.meta.url));
...
async function fixture(mutate) {
  ...
  for (const theme of THEMES) {
    for (const relative of [
      ...
      const source = relative.startsWith("assets/")
        ? join(RESOURCE_ROOT, ...relative.split("/"))
        : join(ASSET_ROOT, ...relative.split("/"));
      const destination = relative.startsWith("assets/")
        ? join(resources, ...relative.split("/"))
        : join(assets, ...relative.split("/"));
...
test("declares the native slim renderer and six exact skins", async () => {
  ...
  assert.equal(
    manifest.runtimeAssets.some(({ path }) =>
      /geckolib|textures\/entity|shader|\.glb$/u.test(path),
    ),
    false,
  );
});
...
test("rejects a schema-v2 manifest that omits the native renderer declaration", async () => {
  await rejectsFixture(async ({ manifest }) => {
...
test("rejects a malformed skin PNG even when its runtime hash is coordinated", async () => {
  await rejectsFixture(async ({ manifest, resources }) => {
...
test("rejects runtime hash tampering, traversal paths, and unknown resource files", async () => {
  ...
  await rejectsFixture(async ({ resources }) => {
