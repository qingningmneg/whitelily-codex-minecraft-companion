...
2. `BridgeProofStore`：用稳定文件句柄读取一次性请求，拒绝链接、重解析点、超限、非规范整数、非 30 秒 TTL、过期、端口不符或格式异常的记录。消费时不用 Windows 上可能覆盖目标的 `ATOMIC_MOVE`；它以同目录固定名称原子创建不可替换的 hard-link claim，再建立所有权 hard-link 锚点，证明 request/claim/anchor 是同一文件后删除原 request 链接。只有完成该删除的线程可以批准；碰撞或崩溃保持 fail closed。普通成功清理前再次证明 claim/anchor 相互指向同一自有文件，再紧接着顺序删除二者；证明或删除失败时保留剩余阻塞物。按 3.5 的同用户威胁边界，这不宣称在倒数第二条 hard link 删除后仍能抵御同用户恶意程序插入系统调用级替换。
...
Bridge 的一次性证明用于保护 LAN 登录边界，不用于抵御已经取得当前 Windows 用户权限的恶意程序。WhiteLily 假定自己的当前用户数据根和已经验证的 PCL2 实例不会被同一用户权限下的代码持续恶意竞态修改；实现仍必须拒绝预先存在的 link、reparse point、路径逃逸和同名碰撞，并在每个已声明的文件系统边界前后复核身份、原子地拒绝覆盖、只清理自身创建且身份仍匹配的精确文件。

Node 在 Windows 上没有句柄相对的 `openat`/`linkat`/`unlinkat`，普通文件句柄也不能形成不可删除共享锁，因此路径复核只能检测漂移，不能诚实承诺击败一个能够读取本地进程/请求状态、在每个系统调用之间替换父目录、修改 ACL 或终止进程的同用户攻击者。此类本机账户失陷不在本版本安全承诺内；远程 LAN 客户端、未确认连接、预置重解析对象、普通碰撞和已检测到的身份变化仍全部 fail closed。
