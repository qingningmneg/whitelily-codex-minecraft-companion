...
`CompanionService` coordinates owner/autonomous work. It uses `ChatRouter`, `TaskController`, `TurnToolBudget`, `MinecraftPort`, `ActionExecutor`, memory/state, and the Codex port. It owns two Codex threads: the intent-classification thread has no Minecraft tools, while the execution thread receives the reviewed `minecraft_*` functions as non-deferred app-server `dynamicTools`. Ordinary conversation therefore stays tool-free, and a clear validated task can call its authorized action directly without asking the owner to choose between chatting and acting.

The dynamic-tool adapter and the local MCP server reuse the same constrained `ToolRegistry`; neither implements an independent execution path. The registry validates the original Zod schema and live turn lease, obtains trusted context and risk classification, consumes `TurnToolBudget`, and dispatches to `ActionExecutor`. ActionExecutor performs SafetyEngine policy evaluation and invokes `MinecraftPort`. The MCP registry does not depend directly on TaskController, and neither Codex transport can bypass these boundaries.

WhiteLily pins the process-local MCP URL in the private app-server launch arguments. Because the desktop account/model flow may create app-server before the Minecraft runtime starts, the runtime calls `config/mcpServer/reload` after MCP readiness succeeds. Codex 0.145.0 does not reliably inject deferred MCP discoveries into the next Responses request, so live game execution uses the execution thread's dynamic functions and handles app-server `item/tool/call` requests on the existing bidirectional JSON-RPC transport. Unknown, malformed, non-execution-thread, or failed calls return bounded failures instead of hanging.

Dependency direction is therefore: RuntimeFacade -> WhiteLilyAppLifecycle -> CompanionService/Codex/MCP/Minecraft composition; CompanionService -> TaskController and TurnToolBudget; Codex dynamic-tool adapter and MCP server -> shared ToolRegistry -> TurnToolBudget, trusted context/classification, and ActionExecutor (not TaskController); ActionExecutor -> SafetyEngine and MinecraftPort; MineflayerAdapter -> MineflayerConnection and `createWorldSnapshot`. The arrows always point toward lower-level implementation or policy boundaries, never back toward the facade.
